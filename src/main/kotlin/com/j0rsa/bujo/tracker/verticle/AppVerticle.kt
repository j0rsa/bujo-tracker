...
import com.j0rsa.bujo.tracker.handler.ActionHandler.createWithTags
...
class AppVerticle : CoroutineVerticle() {
	private val commandHandler = object : CommandSyntax {
		override val vertx: Vertx = getVertx()
	}
			...
			if (asyncResult.succeeded()) {
				...
				asyncResult.result().apply {
					val operationHandlers = mapOf<String, Handler<RoutingContext>>(
						...
						"createAction" to { commandHandler.createWithTags(it) },
	...
	private infix fun String.to(fn: KFunction1<Vertx, suspend (RoutingContext) -> Either<TrackerError, Response<*>>>): Pair<String, Handler<RoutingContext>> =
		this to coroutineHandler {
			fn(vertx)(it)
		}

	private infix fun String.to(fn: (RoutingContext) -> Either<TrackerError, Response<*>>): Pair<String, Handler<RoutingContext>> =
		this to coroutineHandler {
			fn(it)
		}
